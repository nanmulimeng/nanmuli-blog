...
const canConvert = computed(() => task.value?.status === 3)
    ...
    <template v-if="task">
      <div class="mb-6 flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          ...
          <el-button v-if="task.status === 4" type="warning" :icon="Refresh" @click="handleRetry">
            重试
          </el-button>
          <el-button v-if="isTerminal" type="danger" :icon="Delete" plain @click="handleDelete">
            删除
          </el-button>
        </div>
      </div>
      ...
      <div class="mb-6 rounded-xl border border-border bg-surface-secondary p-6 shadow-sm">
        <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            ...
            <div class="flex items-center gap-2">
              <el-progress :percentage="task.progressPercent" :stroke-width="10" :color="progressColor" class="flex-1" />
              <span class="text-sm font-medium text-content-primary">{{ task.completedPages }}/{{ task.totalPages }}</span>
            </div>
          </div>
        </div>
      ...
      <div v-if="task.aiTitle || task.aiSummary" class="mb-6 rounded-2xl p-6 glass-card">
        ...
        <div v-if="task.aiTitle" class="mb-4">
          <div class="mb-1 text-xs text-content-tertiary">标题</div>
          <div class="text-lg font-semibold text-content-primary">{{ task.aiTitle }}</div>
        </div>

        <div v-if="task.aiSummary" class="mb-4">
          <div class="mb-1 text-xs text-content-tertiary">摘要</div>
          <div class="text-sm leading-relaxed text-content-secondary">{{ task.aiSummary }}</div>
        </div>

        <div v-if="task.aiKeyPoints?.length" class="mb-4">
          <div class="mb-1 text-xs text-content-tertiary">关键点</div>
        ...
        <div v-if="task.aiTags?.length" class="mb-4">
          <div class="mb-1 text-xs text-content-tertiary">标签</div>
        ...
        <div v-if="task.aiCategory" class="mb-4">
          <div class="mb-1 text-xs text-content-tertiary">分类</div>
          <div class="text-sm text-content-primary">{{ task.aiCategory }}</div>
        </div>

        <div
          v-if="aiSearchMetadata && (aiSearchMetadata.originalKeyword || aiSearchMetadata.optimizedKeyword || aiSearchVariants.length)"
          class="mb-4 rounded-lg border border-border bg-surface-tertiary/50 p-4"
        >
          <div class="mb-3 text-sm font-medium text-content-primary">AI Search Metadata</div>

          <div v-if="aiSearchMetadata.originalKeyword" class="mb-3">
            <div class="mb-1 text-xs text-content-tertiary">Original Keyword</div>
            <div class="text-sm text-content-primary">{{ aiSearchMetadata.originalKeyword }}</div>
          </div>

          <div v-if="aiSearchMetadata.optimizedKeyword" class="mb-3">
            <div class="mb-1 text-xs text-content-tertiary">Optimized Keyword</div>
            <div class="text-sm text-content-primary">{{ aiSearchMetadata.optimizedKeyword }}</div>
          </div>

          <div v-if="aiSearchVariants.length">
            <div class="mb-1 text-xs text-content-tertiary">Search Variants</div>
            <div class="flex flex-wrap gap-2">
              <el-tag v-for="variant in aiSearchVariants" :key="variant" size="small" effect="plain">
                {{ variant }}
              </el-tag>
            </div>
          </div>
      ...
      <div v-if="pages.length > 0" class="rounded-2xl p-6 glass-card">
        <h3 class="mb-4 text-lg font-semibold text-content-primary">爬取页面 ({{ pages.length }})</h3>
            ...
            <div class="space-y-3">
              <div class="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
                ...
                <div>
                  ...
                  <el-tag :type="page.crawlStatus === 2 ? 'success' : page.crawlStatus === 3 ? 'danger' : 'info'" size="small">
                ...
                <div>
                  ...
                  <div class="text-sm text-content-primary">
                    {{ page.wordCount?.toLocaleString() || '-' }} 字 / {{ formatDuration(page.crawlDuration) }}
                  </div>
                </div>
              </div>
              ...
              <div v-if="page.rawMarkdown" class="rounded-lg border border-border bg-surface-tertiary/50 p-4">
                ...
                <div
                  v-else
                  class="line-clamp-6 whitespace-pre-wrap font-mono text-sm leading-relaxed text-content-secondary"
                >
                  {{ page.rawMarkdown }}
                </div>
              </div>
